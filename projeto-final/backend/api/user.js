...
module.exports = app => {
    ...
    const get = (req, res) => {
        app.db("users").select("id", "name", "email", "admin").then(users => res.json(users)).catch(err => res.status(500).send(err))
    }
    
    const getByID = (req, res) => {
        app.db("users").select("id", "name", "email", "admin").where({ id: req.params.id }).then(user => res.json(user)).catch(err => res.status(500).send(err))
    }
