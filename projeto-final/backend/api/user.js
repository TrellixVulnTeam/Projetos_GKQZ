...
module.exports = app => {
    ...
    const save = async (req, res) => {
        ...
        try {
            existsOrError(user.name, "Nome não cadastrado.")
        ...
        if (user.id) {
            app.db("users").update(user).where({ id: user.id }).whereNull("deletedAt").then(_ => res.status(204).send()).catch(err => res.status(500).send(err))
        } else {
    ...
    const get = (req, res) => {
        app.db("users").select("id", "name", "email", "admin").whereNull("deletedAt").then(users => res.json(users)).catch(err => res.status(500).send(err))
    }

    const getByID = (req, res) => {
        app.db("users").select("id", "name", "email", "admin").where({ id: req.params.id }).whereNull("deletedAt").then(user => res.json(user)).catch(err => res.status(500).send(err))
    }
